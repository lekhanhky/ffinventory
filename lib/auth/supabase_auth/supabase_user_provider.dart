...
class MyInventorySupabaseUser extends BaseAuthUser {
  MyInventorySupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> myInventorySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MyInventorySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
